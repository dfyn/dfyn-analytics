  ...
  const ListItem = ({ item }) => {
    return (
      <DashGrid style={{ height: '48px' }}>
        ...
        <DataText area="value">
          {currency === 'OKT' ? 'Ξ ' + formattedNum(item.valueETH) : formattedNum(item.amountUSD, true)}
        </DataText>
        {!below780 && (
          <>
        ...
        {!below1080 && (
          <DataText area="account">
            <Link color={color} external href={'https://www.oklink.com/okexchain/address/' + item.account}>
